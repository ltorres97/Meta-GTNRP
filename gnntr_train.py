...
def sample_train(data, batch_size, n_support, n_query):
...
def sample_test(data, batch_size, n_support, n_query):
    
    test_task = 1 # replace with test task
    
    dataset = MoleculeDataset("Data/" + data + "/pre-processed/task_" + str(test_task), dataset = data)
    support_dataset, query_dataset = random_sampler(dataset, data, test_task-1, n_support, n_query, train=False, task="bin")
    support_set = DataLoader(support_dataset, batch_size=batch_size, shuffle=False, num_workers = 0, drop_last=True)
    query_set = DataLoader(query_dataset, batch_size=batch_size, shuffle=False, num_workers = 0, drop_last = True)
...
class GNNTR():
    ...
    def meta_train(self):
        ...
        support_sets, query_sets = sample_train(self.data, self.batch_size, self.n_support, self.n_query)
    ...
    def meta_test(self):
        ...
        for test_task in range(self.test_tasks):
            ...
            support_set, query_set = sample_test(self.data, self.batch_size, self.n_support, self.n_query)
            self.gnn.eval()
            if self.baseline == 0:
